...
const TransactionItem = ({ item, onDelete }) => {
    // console.log('TransactionItem received:', item);
        ...
        <View style={styles.transactionCard} key={item.id}>
            <TouchableOpacity style={styles.transactionContent}>
                <View style={styles.categoryIconContainer}>
                    <Ionicons name={iconName} size={20} color={isIncome ? COLORS.income : COLORS.expense} />
                </View>
                ...
                <View style={styles.transactionRight}>
                    <Text style={[styles.transactionAmount, { color: isIncome ? COLORS.income : COLORS.expense }]}>
                        {isIncome ? '+' : '-'} Ksh {Math.abs(parseFloat(item.amount).toFixed(2))}
                    </Text>
